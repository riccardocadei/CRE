...
#' @param rules_method the method for selecting causal rules with binary outcomes, either "conservative", "anticonservative", or NA
#'
...
select_causal_rules <- function(rules_matrix_std, rules_list, ite_std, binary, q, rules_method) {
  ...
  if (binary) {
    ...
    stab_mod <- stabs::stabsel(rules_matrix_std, ite_std, fitfun = "glmnet.lasso", cutoff = q,
                               PFER = 1, args.fitfun = list(type = rules_method))
