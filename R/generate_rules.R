...
generate_rules <- function(X, ite_std, ntrees, min_nodes, max_nodes) {
  ...
  # Random Forest
  forest <- randomForest(x = X, y = ite_std, sampsize = sf * N, replace = FALSE, ntree = 1, maxnodes = mn, nodesize = min_nodes)
  for(i in 2:ntrees) {
    mn <- 2 + floor(rexp(1, 1 / (max_nodes - 2)))
    model1_RF <- randomForest(x = X, y = ite_std, sampsize = sf * N ,replace = FALSE, ntree = 1, maxnodes = mn, nodesize = min_nodes)
    forest <- combine(forest, model1_RF)
  }
  treelist_RF <- RF2List(forest)
  ...
  model1_GB <- gbm.fit(x = X, y = ite_std, bag.fraction = sf, n.trees = 1, interaction.depth = (mn / 2),
                       shrinkage = 0.01, distribution = dist, verbose = FALSE, n.minobsinnode = min_nodes)
  for(i in 2:ntrees) {
    model1_GB$interaction_depth <- (mn / 2)
    model1_GB <- gbm.more(model1_GB, n.new.trees = 1, verbose = FALSE)
  }
  treelist_GB <- GBM2List(model1_GB, X)
  rules_GB <- extract_rules(treelist_GB, X, ntrees, ite_std)
