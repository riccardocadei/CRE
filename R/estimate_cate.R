...
estimate_cate <- function(ite_inf, rules_matrix_inf, rules_list_inf) {
  ...
  cate_means <- data.frame(Rule = "Average Treatment Effect", CATE = mean(ite_inf),
                           CI_lower = mean(ite_inf) - (1.96 * sqrt(stats::var(ite_inf)) / sqrt(length(ite_inf))),
                           CI_upper = mean(ite_inf) + (1.96 * sqrt(stats::var(ite_inf)) / sqrt(length(ite_inf))))

  # Determine CATE manually for each rule
  for (i in 1:length(rules_list_inf)) {
    df_temp <- data.frame(ite = joined_ite_rules[,1], rule = joined_ite_rules[,i + 1]) %>% dplyr::filter(rule == 1)
    cate_temp <- data.frame(Rule = rules_list_inf[i], CATE = mean(df_temp$ite),
                            CI_lower = mean(df_temp$ite) - (1.96 * sqrt(stats::var(df_temp$ite)) / sqrt(length(df_temp$ite))),
                            CI_upper = mean(df_temp$ite) + (1.96 * sqrt(stats::var(df_temp$ite)) / sqrt(length(df_temp$ite))))
  ...
  cate_reg_orig_names <- stringr::str_extract(row.names(cate_reg_orig), "`.*`") %>% stringr::str_remove_all("`")
  cate_reg_orig_names[1] <- "(Intercept)"
  cate_reg_orig$Rule <- cate_reg_orig_names
  row.names(cate_reg_orig) <- 1:nrow(cate_reg_orig)
  cate_reg_orig <- cate_reg_orig %>% dplyr::summarize(Rule, CATE = Estimate, PVal = `Pr(>|t|)`,
