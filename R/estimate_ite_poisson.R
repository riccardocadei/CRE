...
estimate_ite_poisson <- function(y, z, X, X_names, include_offset, offset_name) {
  if (include_offset) {
    ...
    temp1 <- stats::glm(y_treated ~ offset(log(offset_treated)) + X_treated, data = model_data_treated, family = poisson(link = "log"))
    temp0 <- stats::glm(y_control ~ offset(log(offset_control)) + X_control, data = model_data_control, family = poisson(link = "log"))
  } else {
    y_treated <- data.frame(y = y[z==1])
