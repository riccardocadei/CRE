...
We recommend using `or`, `bcf`, `cf`, or `xbcf` on data with **continuous** outcomes and `or`, `bart`, `xbart` or `cf` on data with **binary** outcomes to maximize efficiency and accuracy. The following ITE estimation methods are **NOT** compatible with binary outcomes: `ipw`, `sipw`, `bcf`, and `xbcf`.
